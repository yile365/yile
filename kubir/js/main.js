...
//移动端移植：良凉 https://gitee.com/liangfm
...
//var IS_IPAD = (IsPC())?false:true; 用使设备判定
var IS_IPAD = true;
...
var StateVectorComponent = React.createClass({
    ...
    render:function(){
        ...
        return  <div className = "stateVector" onMouseEnter = {this.showMsg} style = {{background:getColor.bind(this,Math.ceil(state.amount))()}}>
                    {stateName}:{this.props.state == 'temp'?TEMP_DATA[this.context.getTempDesc()].name:Math.ceil(state.amount)}
                </div>
    }
});
...
var MenuBtnComponent = React.createClass({
    ...
    render:function(){
        ...
        return <BtnComponent style = {{backgroundColor:'#9FAA83'}} className = 'stateVector' handleClick = {this.handleClick}><span>技能</span>{(menuHint>0?<span className = 'badge'>{menuHint}</span>:'')}</BtnComponent>
    }
});
    ...
    render:function(){
        ...
        function getSkillList(){
            ...
            for(var attr in skill){
                ...
                result.push(<div className = 'btn skillItem' onClick = {this.handleSkillTab.bind(this,attr)} key = {SKILL_DATA[attr].name}>{SKILL_DATA[attr].name} &nbsp; {SKILL_DATA[attr].one?null:<span className = 'badge'>{skill[attr]}级</span>}</div>)
            }
            return result;
        }
        ...
        return <div className = 'menuOuter'>     
                    ...
                    <div className = 'panel-primary'>
                        ...
                        <div><BtnComponent desc = '关闭窗口' handleClick = {this.quitMenu}/></div>
                    </div>   
                </div>
    }
});
...
var UpgradePlaceComponent = React.createClass({
    ...
    render:function(){
        if(this.state.teacher){
            return  <div><TeacherComponent type = {this.state.teacher}/>
...
var BagComponent = React.createClass({
    ...
    componentWillMount:function(){
        ...
        var boxSaveData = this.context.boxSaveData;
        boxSaveData['bag'].size = BAG_BASE_SIZE + level*2;
    },
    ...
    render:function() {
        ...
        function getItemDetail(){
            ...
            if(equipType){
                if(currentEquip[equipType] != detailedItem){
                    equipShow = <p style = {{color:COLOR.RED}}>未装备</p>;
                }else{
                    equipShow = <p style = {{color:COLOR.GREEN}}>已装备</p>;
                }
            }
            function getDetailDesc(){
                if(ITEM_DATA[detailedItem].effect){
                    ...
                    for (var attr in ITEM_DATA[detailedItem].effect) {
                        ...
                        res.push(<span key = {attr} className = {(isGreen?"effectPlus":"effectMinus")}>{STATE_DATA[attr].name}: {prefix}{effectAmount} </span>)
                    };
                    return <p>{res}</p>
                }else{
                    return null
            ...
            return  <div className = "detailHead">
                        <span>{ITEM_DATA[detailedItem].name} / </span>{equipType?<span>{EQUIP_TYPE_DATA[equipType]}</span>:null}<span>{equipShow || TYPE_DATA[ITEM_DATA[detailedItem].type].name} </span>
                        {maxDurable != undefined?<p>耐久度：{maxDurable - durable}/{maxDurable} </p> : null}
                        {getDetailDesc.bind(this)()}
                        <p className = "clearFix">{ITEM_DATA[detailedItem].desc}</p> 
        ...
        function getStateDetail(){
            ...
            // 背包-属性说明
            return  <div className = "detailHead">
                        <span className = ' clearFix'>{name}</span>
...
var TrampComponent = React.createClass({
    ...
    render:function(){
        ...
        if(!this.state.over){
            return <div>
                        <p>{header}</p>
                        {value == 0?(getLength(playerGive) == 0?<p>我上有老下有小，等着救命！能给我8枚水果么？</p>:<p>拜托..我要食物啊...</p>):(foodGot + value> this.maxFood ? <p>太感谢了，上帝保佑你...</p> :<p>谢谢你...还有吗...</p>)}
                        <RegisterComponent itemList = {{}} canBack = {false} onlyOne = {true} canBeEmpty = {true} canPick = {false}/>
                        {value == 0?<BtnBack/>:<BtnComponent handleClick={this.giveFood} desc = {'帮助'} />}
                    </div>;
            }else{
                if(foodGot > this.maxFood){
                    return <div>
                                ...
                                <p>我感觉好多了。</p>
                                <p>--------</p>
                                <p>村民递给你一张<span style = {{color:COLOR.GREEN}}>[小镇]</span>的地图。</p>
                                <p>村民扬长而去....</p>
                                <BtnBack/>
                            </div>;
                }else{
        ...
        if(!choice){
            return(
                    <div>
                        ...
                        <p style = {{color:'#C2C788'}}>{townDesc}{town == 'ice'?<span>擅长格斗，你将习得 ‘嗜血’技能 战斗胜利时，将恢复30%的生命</span>:<span>擅长操控魔法，你可习得 ‘吸收’技能 战斗胜利时，将恢复30%的精神</span>}</p>
...
var TrapComponent = React.createClass({
    ...
    componentWillMount:function(){
        ...
        var buildingSaveData = this.context.buildingSaveData;
        buildingSaveData['trap'].size = 2 + level*2;
    },
    getInitialState:function(){
        return {
            size:null
    ...
    getAmount:function(itemAmount){
        ...
        return Math.round(itemAmount * (1 + 0.5 * level*10));
    },
...
var WellComponent = React.createClass({
    ...
    render:function(){
        ...
        var yieldAmount = (3 + level*2)* ((this.context.skill.manage || 0)*SKILL_DATA.manage.buff + 1);
        ...
        return  <div>
                    <p>水井 <span style = {{color:COLOR.BLUE}}>{level}级</span></p>
...
var BattleCharactorComponent = React.createClass({
    ...
    render:function(){
        ...
        return  <div className = "battleCharactor you">
                    ...
                    <p>{this.getPrefix()}{this.props.name} : {Math.ceil(currentHp)}/{Math.ceil(maxHp)}</p>           
                </div>
    }
});
    ...
    render:function(){
        ...
        function getWeaponRow(index){
            ...
            for (var index = 0; index < 2; index++) {
                ...
                result.push(<tr key = {index}>
                                <td>手<SelectComponent defaultV = {selectedWeapon} handleChange = {this.handleChange} index = {index}>{getOptions(index)}</SelectComponent></td>
                                ...
                                <td><span className = 'damageb'>{this.props.getDamage(selectedWeapon)}</span> vs <span className = 'damage'>{this.props.mstDmg}</span></td>
        ...
        function getRunRow(index){
                return  <tr>
                            ...
                            <td></td>
                            <td><span className = 'damage'>失败 生命(-{this.props.mstDmg})</span></td>
                            <td><BtnComponent desc = '逃跑' handleClick = {this.props.handleRun}/></td>
                        </tr>
        }
        return <div>
                    <div className = "tableOuter">
...
var BattleComponent = React.createClass({
    ...
    handleAttack:function(weapon){
        ...
        function getMstDo(){
            if(mst_feared)return <span>害怕得动弹不了！</span>;
    ...
    handleRun:function(){
        ...
        function callBack(){
            ...
            if(Math.random() < chance){
                ...
                var show = <p key = {'run_'+this.step}>逃跑失败！你受到了<span style = {{color:COLOR.RED}}>{enermyDmg}</span>点伤害！</p>;
...
var DungeonComponent = React.createClass({
    ...
    render:function(){
        ...
        return(
                <div style = {{margin:'auto',width:450,height:300}}>
                <div className = "dungeonHeading">
                    <p>第 <span className="num">{dungeonSaveData.stairCount}</span> 层 <span className="num">{dungeonSaveData.stairCount}0{dungeonSaveData.roomCount}</span> 室</p>
                    <p>最高记录: <span className="num">{dungeonSaveData.deepest}</span>层</p>
        ...
        return <div className="panel panel-primary advanMain" >
                <div className="panel-heading">
                    <div className = "time" id = "time">
                        ...
                        {seasonDescMap[this.context.season]}第{time.day}日 {getTimeDiplay(time.hour)}
                    </div>
                    <span className = 'season' style = {{color:COLOR.BLUE}}>在{this.state.title}</span>
                </div>
...
var MainComponent = React.createClass({
    ...
    handleDeath:function(type){
        ...
        var desc = (
                <div style = {{margin:100}}>
                    ...
                    <p>死因：{reason}</p>
                    <p>提示：从新开始吧我的英雄！！</p>
                    <BtnComponent handleClick = {this.init}>重新挑战</BtnComponent>
                </div>
            )
        this.setState({
            menuDesc:desc,
    ...
    getMaxDurable:function  (item) {
        ...
        return Math.round(durable * (1 + level * 1));
    },
